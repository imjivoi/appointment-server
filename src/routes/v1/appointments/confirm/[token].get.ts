...
export default defineEventHandler(async (event) => {
  const { token } = await useValidatedParams(event, ConfirmAppointmentSchema);
  ...
  if (updateAppointmentError) {
    ...
    return sendError(
      event,
      createError({
        statusMessage: "Can not confirm appointment",
        statusCode: 500,
      })
    );
  }
...
const ConfirmAppointmentSchema = v.objectAsync({
  token: v.string(),
});
